...
void check_range(cm **carte , enn *ennemis , sh *tirs)
{
	...
	for (int i = 1 ; i < 16 ; i++)
	{
		for (int j = 1 ; j < 16 ; j++)
		{
			...
			{
				x = -1;
				x = focus(ennemis , carte[i][j].tr , tirs);	
				if (x != -1)
				{
...
int focus(enn *ennemis , tower tr, sh *tirs)
{
	...
	for (int i = 0; i < 600 ; i++)
	{
		if (ennemis[i].active == 1)
		{
			if (sqrt(pow(tr.c.x - ennemis[i].c.x, 2) + pow(tr.c.y - ennemis[i].c.y, 2)) <= tr.range)
			{
				if (check_tir_dmg_suffisant(ennemis , i , tirs) == 0 && check_tir_sup_pa(ennemis[i] ,tr) == 1)
				{
					if (foc == -1)
					{
						foc = i;
					...
					{
						if (ennemis[i].chem > ennemis[foc].chem)
							foc = i;
					}
				}
			}
...
void creation_vague(vague *vag)
{
	...
	{
		vag[i].lvl = i + 1;
		vag[i].nb_soldat = nb_soldat_vague(vag[i].lvl);
		vag[i].nb_tank = nb_tank_vague(vag[i].lvl);
...
void erreurs(int lecture , int chemin)
{
	if (lecture == 1)
	{
		...
		if (chemin == 0)
			printf("Erreur : Le chemin n'est pas relié.\n");
		if (chemin == -1)
			printf("Erreur : Le niveau n'est pas entouré d'eau.\n");
	}
	if (lecture == 0)
		printf("Erreur : Trop de caractère de le fichier niveau.\n");
	if (lecture == -1)
		printf("Erreur : Pas de début ni de fin.\n");
	if (lecture == -2)
		printf("Erreur : Pas de début.\n");
	if (lecture == -3)
		printf("Erreur : Pas de fin.\n");
}
